...
#have to split the given data as training and validation set 
...
class Training(object):
    ...
    def __init__(self, batch_size,nb_epoch,load_model_resume_training=None):
        ...
        #self.model = Sequential() #added because we first have to define the architeture
    ...
    def fit_unet(self,X33_train,Y_train,X_patches_valid=None,Y_labels_valid=None):
        ...
        checkpointer = ModelCheckpoint(filepath='ResUnet.{epoch:02d}_{val_loss:.3f}.hdf5', verbose=1)
...
if __name__ == "__main__":
    ...
    #np.resize(a,(2,3))
    #Y_labels = np.resize(Y_labels, (1000, 128, 128, 4))
    Y_labels = np.resize(Y_labels, (5, 128, 128, 4))
    X_patches=np.load("x_training.npy").astype(np.float32)
    #X_patches = np.resize(X_patches, (1000, 128, 128, 4))
    X_patches = np.resize(X_patches, (5, 128, 128, 4))


    #Y_labels_valid=np.load("y_valid.npy").astype(np.uint8)
    #X_patches_valid=np.load("x_valid.npy").astype(np.float32)

    #Mike: using train_test_split to split the data for training and validation
    X_patches, X_patches_valid, Y_labels, Y_labels_valid = train_test_split(X_patches, Y_labels, test_size=0.20, random_state=42)
